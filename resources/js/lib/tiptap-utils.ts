...
export const handleImageUploadLocal = async (
  file: File,
  onProgress?: (event: { progress: number }) => void,
  abortSignal?: AbortSignal
): Promise<string> => {
...
export const handleImageUploadCloud = async (
  file: File,
  onProgress?: (event: {progress: number }) => void,
  abortSignal?: AbortSignal
): Promise<string> => {
  ...
  formData.append("upload_preset", "cavitetours_uploads");

  const response = await fetch("https://api.cloudinary.com/v1_1/dyxohkhij/image/upload" , {
    method: "POST",
    body: formData,
    signal: abortSignal
  });
